...
public class FireLook : NetworkBehaviour {
    [SyncVar]
    public float xRot;
    [SyncVar]
    public float yRot;
    ...
    private Quaternion m_HeroTargetRot;
    private Quaternion m_HeroCatWalkTargetRot;
    private bool m_cursorIsLocked = true;
    private Health HeroHealth;
    public LockFire lockfire;
    ...
    public float followanglefator = 0.7f;
        public void Init()
        {
            ...
            m_HeroTargetRot = Hero.localRotation;
        ...
        public void RpcLookRotation(float yRot)
        {
            ...
            m_PitchTargetRot *= Quaternion.Euler(0f, 0f, yRot);

            if (clampVerticalRotation)
            {   
                m_PitchTargetRot = ClampRotationAroundZAxis(m_PitchTargetRot,MinimumPitch,MaximumPitch);
            }
        ...
        m_YawTargetRot *= Quaternion.Euler(0f, xRot, 0f);
            m_HeroTargetRot *= Quaternion.Euler(0f, xRot, 0f);
            if (clampVerticalRotation)
            {
                m_YawTargetRot = ClampRotationAroundYAxis(m_YawTargetRot, MinimumYaw, MaximumYaw);
                m_HeroTargetRot = ClampRotationAroundZAxis(m_HeroTargetRot, -100, 100);
            ...
            if (smooth)
            {
                    ...
                    {
                        Hero.localRotation = Quaternion.Slerp(Hero.localRotation, m_HeroTargetRot,
                        smoothTime * Time.deltaTime);
        ...
        void Turn()
        {
            ...
            if (turnleft)
            {
                m_HeroTargetRot *= Quaternion.Euler(0f, -turnangle, 0f);
                Hero.localRotation = Quaternion.Slerp(Hero.localRotation, m_HeroTargetRot,
    ...
    void RpcCatWalk()
    {
        if (catwalkflag)
        {
            if (catwalkstep == 0)
            {
                ...
                m_HeroCatWalkTargetRot = m_HeroTargetRot * Quaternion.Euler(0f, 0f, 0f);
                Hero.localRotation = Quaternion.Slerp(Hero.localRotation, m_HeroCatWalkTargetRot,
            ...
            if (catwalkstep == 1)
            {
                ...
                Yaw.localRotation = Quaternion.Slerp(Yaw.localRotation, m_YawTargetRot,
                catwalksmoothtime * followtimefator * Time.deltaTime);
                m_HeroCatWalkTargetRot = m_HeroTargetRot * Quaternion.Euler(0f, -catwalkangle, 0f);
                Hero.localRotation = Quaternion.Slerp(Hero.localRotation, m_HeroCatWalkTargetRot,
                catwalksmoothtime * Time.deltaTime);
